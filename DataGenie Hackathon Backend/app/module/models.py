...
# def generate_tbats_forecast(data, date_from, date_to, frequency, period):
#     # Filter data based on the specified date range
#     training_data = data[date_from:]
#     date_from_ts = datetime.strptime(date_from, "%Y-%m-%dT%H:%M:%S.%f%z")
#     date_to_ts = datetime.strptime(date_to, "%Y-%m-%dT%H:%M:%S.%f%z")
#     print("Frequency:", frequency)
    
#     # Fit TBATS model to the filtered data
...
#     # Calculate forecast period based on frequency
#     if frequency == 'H':
#         forecast_period = (date_to_ts - date_from_ts).total_seconds() / 3600  # Difference in hours
#     elif frequency == 'D':
#         forecast_period = (date_to_ts - date_from_ts).days  # Difference in days
#     elif frequency == 'W':
#         forecast_period = ((date_to_ts - date_from_ts).days + 1) // 7  # Difference in weeks
#     elif frequency == 'M':
#         forecast_period = (date_to_ts.year - date_from_ts.year) * 12 + date_to_ts.month - date_from_ts.month  # Difference in months
#     elif frequency == 'Y':
#         forecast_period = date_to_ts.year - date_from_ts.year  # Difference in years
#     forecast_period+=1
#     print("Forecast Period:", forecast_period)
    
#     # Make predictions for the specified period
#     forecast = model.forecast(steps=int(forecast_period))
#     forecast_index = pd.date_range(start=date_from_ts, periods=int(forecast_period), freq=frequency)
#     forecast = pd.Series(data=forecast.flatten(),index=forecast_index)
#     forecast = forecast.astype('float')
#     print("Forecast\n", forecast)
...
#     # Calculate MAPE
#     actual_values = data.loc[date_from_ts:date_from_ts + pd.DateOffset(days=forecast_period-1)]  # Actual values for the specified date range
#     result_json = create_result_json(forecast, actual_values)
#     mape = mean_absolute_percentage_error(actual_values, forecast)
